...
def expand(left, right): # 판별 조건에 맞춰 확장할 포인터를 지정해주는 함수 정의
    while left >= 0 and right < len(example) and example[left] == example[right]: 
        ...
        #                     여기서 left, right 포인터는 인덱스로 쓰이기 때문에 예를 들어 길이가 5인 단어의 마지막 포인터는 인덱스로는 4이기 때문에 5일 수가 없다
    ...
    return example[left+1 : right] # while문이 false가 될 때 escape 하여 반환값을 내놓는다
                            # left는 위에서 한 번 감소하였을 때 조건에 충족 되지 않고 끝났기 때문에 다시 +1, right는 +1 되었어도, 어차피 슬라이싱에선 끝자리 포함이 되지 않으므로 right로만 쓰면 된다

if len(example) < 2 or example == example[::-1]: # 단어의 길이가 1일때 e.g. example = "a"이면 그냥 a만 반환하면 되고, reverse 했을 때 같은 경우엔 단어 전체가 가장 긴 팰린드롬임이 충족되므로 그냥 반환
    print(example)
